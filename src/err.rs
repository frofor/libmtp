...
impl Error {
	...
	pub(crate) fn new(kind: ErrorKind, text: &str) -> Self {
	...
	pub(crate) fn from_ffi(ptr: *const ffi::LIBMTP_error_t) -> Option<Self> {
		...
		let kind = MtpErrorKind::new(err.errornumber)?;
		let text = unsafe { ptr_to_string(err.error_text) };
		Some(Self { kind: ErrorKind::Mtp(kind), text })
	}
}
...
pub enum ErrorKind {
	/// An error occured at the MTP (Media Transfer Protocol) layer.
	Mtp(MtpErrorKind),
	/// Input/output error.
	Io,
	/// Unknown error.
...
impl MtpErrorKind {
	...
	pub(crate) fn new(n: ffi::LIBMTP_error_number_t) -> Option<Self> {
		match n {
			ffi::LIBMTP_error_number_t::None => None,
			ffi::LIBMTP_error_number_t::General => Some(Self::General),
			ffi::LIBMTP_error_number_t::PtpLayer => Some(Self::Ptp),
			ffi::LIBMTP_error_number_t::UsbLayer => Some(Self::Usb),
